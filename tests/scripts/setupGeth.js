...
let gethSetupConfig = gethManager.getGethSetupConfig(),
  ...
  poaGenesisOutPath =
    ...
    path.resolve(__dirname, rootPrefix + '/tests/scripts/poa-genesis.json'),
  gethIgnoreArgs = gethSetupConfig['preInitArgsToIgnore'] || [
    'etherbase',
    'unlock',
    'password',
    'mine',
  ...
  passphraseFilePath =
    gethSetupConfig['passphraseFilePath'] || path.resolve(__dirname, rootPrefix + '/tests/scripts/pw'),
    ...
    if (!fs.existsSync(datadir)) {
      ...
      rmProcess.on('exit', function(code, signal) {
        if (!code) {
          ...
          let err = new Error(
            'Could not clean datadir directory. rm exit code: ' + code + '. command: rm ' + rmArgs.join(' ')
          );
          setTimeout(function() {
            reject(err);
          }, 0);
